...
class PathOptimizer:
    ...
    def optimize_spiral(self, xf, yf, tf):
        ...
        # ------------------------------------------------------------------
        bounds = [[-0.5, -0.5, sf_0], [0.5, 0.5, float('inf')]]
        # ------------------------------------------------------------------
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        # ...
        thetas = a * s + np.multiply(b / 2 * s, s) + np.multiply(np.multiply(c / 3 * s, s), s) + np.multiply(np.multiply(np.multiply(d / 4 * s, s), s), s)
        return thetas
        # ------------------------------------------------------------------
    ...
    def sample_spiral(self, p):
        ...
        #cumulate t_points
        for index in range(len(t_points) - 2):
            t_points[index + 1] = t_points[index + 1] + t_points[index]

        x_points = scipy.integrate.cumtrapz(x, s_points, initial=0)
        y_points = scipy.integrate.cumtrapz(y, s_points, initial=0)
