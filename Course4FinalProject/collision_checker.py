    ...
    def collision_check(self, paths, obstacles):
        ...
        for i in range(len(paths)):
            ...
            for j in range(len(path[0])):
                ...
                # path[1][j]. 
                circle_locations = np.zeros((len(self._circle_offsets), 2), dtype=float)
    ...
    def select_best_path_index(self, paths, collision_check_array, goal_state):
        ...
        for i in range(len(paths)):
            ...
            if collision_check_array[i]:
                ...
                # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                # --------------------------------------------------------------
                d_x = np.subtract(paths[i][0], goal_state[0])
                d_y = np.subtract(paths[i][1], goal_state[1])
                # square error
                score = np.dot(d_x, d_x) + np.dot(d_y, d_y)
                # --------------------------------------------------------------
                ...
                for j in range(len(paths)):
                    if j == i:
                        ...
                        if not collision_check_array[j]:
                            # TODO: INSERT YOUR CODE BETWEEN THE DASHED LINES
                            # --------------------------------------------------
                            d_x = np.subtract(paths[i][0], paths[j][0])
                            d_y = np.subtract(paths[i][1], paths[j][1])
                            score += self._weight * np.dot(d_x, d_x) + np.dot(d_y, d_y)
                            # --------------------------------------------------
